...
def CountingSort(studentList, houseOrYear):
        counting = [0 for i in range(8)]
        output = [0] * len(studentList)
        for j in range(0,len(studentList)):
            key = studentList[j].getIndex(houseOrYear)
        ...
        for j in range(len(studentList)-1,-1,-1):
            key = studentList[j]
            index = counting[key.getIndex(houseOrYear)]
            output[index-1] = key
            counting[studentList[j].getIndex(houseOrYear)] = counting[studentList[j].getIndex(houseOrYear)] - 1
        return output
